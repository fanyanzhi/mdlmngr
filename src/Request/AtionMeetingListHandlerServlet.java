 ...
 */
@WebServlet("/AtionMeetingListHandler.do")
public class AtionMeetingListHandlerServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public AtionMeetingListHandlerServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if ("getcount".equals(request.getParameter("do"))) {
			strResult = getAtionMeetingCount(request);
		} else if ("getlist".equals(request.getParameter("do"))) {
			strResult = getAtionMeetingList(request);
		} else if ("delfile".equals(request.getParameter("do"))) {
			strResult = delAtionMeetingFile(request);
	...
	private String getAtionMeetingCount(HttpServletRequest request) throws ServletException, IOException {
...
//					return String.valueOf(MeetingMngr.getMeetingAllCount(curcode, strUserName, strStartDate, strEndDate));
//				}else if(!curcode.equals(strXueKeCode)){
//					return "0";
//				}
//		}
		return String.valueOf(MeetingMngr.getMeetingAllCount(strXueKeCode, strUserName, strStartDate, strEndDate));
		
	}

	private String getAtionMeetingList(HttpServletRequest request) throws ServletException, IOException {
...
//			String curcode=ZjclsMngr.getCodeByName(strXueKeName);
//			if(Common.IsNullOrEmpty(strXueKeCode)){
//				lstFiles= MeetingMngr.getMeetingList(curcode, strUserName, strStartDate, strEndDate, iStart, iLength);
//			}else if(!curcode.equals(strXueKeCode)){
//				lstFiles= null;
//			}else{
//				lstFiles = MeetingMngr.getMeetingList(strXueKeCode, strUserName, strStartDate, strEndDate, iStart, iLength);
//			}
//		}else{
			lstFiles = MeetingMngr.getMeetingList(strXueKeCode, strUserName, strStartDate, strEndDate, iStart, iLength);
//		}
	...
	private String delAtionMeetingFile(HttpServletRequest request) throws ServletException, IOException {
		String strResult = "";
		...
		if (MeetingMngr.delAttention(strUserName, strFileID)) {
			strResult = "1";
		} else {
			strResult = "0";
		}
	...
	private String getUserCount(HttpServletRequest request) throws ServletException, IOException {
		...
		String strStartDate = request.getParameter("startdate");
		String strEndDate = request.getParameter("enddate");
		return String.valueOf(MeetingMngr.getMeetingUserCount(strXueKeCode, strUserName, strStartDate, strEndDate));
	}
