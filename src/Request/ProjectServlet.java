...
public class ProjectServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		switch (strAction.toLowerCase()) {
			case "/check":
				strRet = checkProjectInfo(strUserName, mapInfo);
				break;
			case "/add":
				strRet = addProjectInfo(appId, strUserName, mapInfo);
				break;
			case "/gets":
				strRet = getProjectList(strUserName);
				break;
			case "/cancel":
				strRet = cancelProject(strUserName, mapInfo);
				break;
			default:
	...
	private String checkProjectInfo(String username,Map<String, Object> arg){
		String sortcode="";
		...
		String count = ProjectMngr.checkAttention(username, sortcode);
		if("1".equals(count)){
	...
	private String addProjectInfo(String appid,String username,Map<String, Object> arg) {
		String sortcode="";
		if (arg.containsKey("sortcode") && arg.get("sortcode") != null) {
			sortcode=String.valueOf(arg.get("sortcode"));
		}
		...
		boolean result =ProjectMngr.addAttention(username, sortcode);
		if(result){
			Map<String, Object> map = ProjectMngr.getLastRecord(username);
			if (map != null)
	...
	private String getProjectList(String UserName){
		List<Map<String, Object>> lstResult = ProjectMngr.getAtionSubjectList(UserName, 1, 7);
	...
	private String cancelProject(String UserName, Map<String, Object> arg) {
		String ids="";
		...
		if (ProjectMngr.delAttention(UserName, ids)) {
			return "{\"result\":true}";
		} else {
			return "{\"result\":false,\"message\":\"".concat(String.valueOf(SysConfigMngr.ERROR_CODE.ERROR_ACCESSDATEBASE.code)).concat("\",\"errorcode\":").concat(String.valueOf(SysConfigMngr.ERROR_CODE.ERROR_ACCESSDATEBASE.code)).concat("}");
		}
