...
/**
 * Servlet implementation class OrgNoticeHandlerServlet
 */
@WebServlet("/OrgNoticeHandler.do")
public class OrgNoticeHandlerServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public OrgNoticeHandlerServlet() {
		super();
	}
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if ("getcount".equals(request.getParameter("do"))) {
			strResult = getOrgNoticeCount(request);
		} else if ("getlist".equals(request.getParameter("do"))) {
			strResult = getOrgNoticeList(request);
		} else if ("delorg".equals(request.getParameter("do"))) {
			strResult = delOrgNotice(request);
		}
	...
	protected String getOrgNoticeCount(HttpServletRequest request) {
		String subjectId = request.getParameter("subjectid");
		int count = OrgNoticeMngr.getOrgNoticeCount(subjectId);
		return String.valueOf(count);
	}

	protected String getOrgNoticeList(HttpServletRequest request) throws ServletException, IOException {
		StringBuilder sbHtml = new StringBuilder();
		String subjectId = request.getParameter("subjectid");
		String subjectName = request.getParameter("subjectname");
		String start = request.getParameter("start");
		String len = request.getParameter("len");
		List<Map<String, Object>> userSignList = OrgNoticeMngr.getOrgNoticeList(subjectId, Integer.parseInt(start),
		...
		while (iMap.hasNext()) {
			mapData = iMap.next();
			sbHtml.append("<td><input name=\"chkorgname\" value=\"").append(mapData.get("orgname"))
	...
	private String delOrgNotice(HttpServletRequest request) throws ServletException, IOException {
		String strResult = "";
		...
		if (OrgNoticeMngr.delOrgNotice(orgname, subjectid)) {
			strResult = "1";
		} else {
			strResult = "0";
		}
