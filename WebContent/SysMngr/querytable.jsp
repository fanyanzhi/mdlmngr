...
<body  onkeydown="if(event.keyCode==13) {LoginLogSearch();return false;}">
		...
		<form id="form1" name="form1">
			...
			<div id="mdlright">
				<div class="mdlrignav">
					...
					库名：<input type="text" id="txtDataBase" name="txtDataBase" class="keyword" />
						&nbsp;&nbsp;&nbsp;表名：<input id="txtTable" name="txtTable" type="text" class="searchkey" />
						<input name="" class="searchbtn" type="button" onclick="querytable();" value="查询" />
					</div>
	...
	function querytable(){
		var varcon = "";
		if ($.trim($("#txtDataBase").val()).length > 0) {
			varcon = varcon + "db=" + $.trim($("#txtDataBase").val()) +"&";;
		}
		if ($.trim($("#txtTable").val()).length > 0) {
			varcon = varcon + "tb=" + encodeURI($.trim($("#txtTable").val())) +"&";;
		}
		$("#hidparam").val(varcon);
		getFormDataCount(1);
	}
